...
public class CacheMediator extends AbstractMediator implements ManagedLifecycle, EnclosedInlinedSequence {
    ...
    private void processResponseMessage(MessageContext synCtx, ConfigurationContext cfgCtx, SynapseLog synLog) {
		...
		if (response != null) {
            ...
            if (contentType.equals(jsonContentType)) {
                ...
                response.setResponseEnvelope(responsePayload);
			...
			if (!cachedResponse.isExpired()) {
                ...
                try {
                    if (cachedResponse.isJson()) {
                        byte[] payload = cachedResponse.getResponseEnvelope();
                    ...
                    if (msgCtx.isDoingREST()) {
                        if ((headerProperties = cachedResponse.getHeaderProperties()) != null) {
                            if (!cachedResponse.isJson()) {
                                omSOAPEnv = SOAPMessageHelper.buildSOAPEnvelopeFromBytes(responseEnvelop,
	 ...
	 * @return global cache
	 */
    public Cache<String, CachableResponse> getMediatorCache() {
        Cache<String, CachableResponse> cache;
        if (mediatorCacheInit.get()) {
            if (CachingConstants.SCOPE_PER_HOST.equals(scope)) {
                cache = Caching.getCacheManagerFactory().getCacheManager(CachingConstants.CACHE_MANAGER)
                        .getCache(org.wso2.carbon.caching.impl.CachingConstants.LOCAL_CACHE_PREFIX +
                                          CachingConstants.MEDIATOR_CACHE);
            } else {
                cache = Caching.getCacheManagerFactory().getCacheManager(CachingConstants.CACHE_MANAGER)
            ...
            CacheBuilder<String, CachableResponse> mediatorCacheBuilder;
            if (CachingConstants.SCOPE_PER_HOST.equals(scope)) {

                mediatorCacheBuilder = cacheManager.createCacheBuilder(
                        org.wso2.carbon.caching.impl.CachingConstants.LOCAL_CACHE_PREFIX +
                                CachingConstants.MEDIATOR_CACHE);
            } else {
                mediatorCacheBuilder = cacheManager.createCacheBuilder(CachingConstants.MEDIATOR_CACHE);
            }
            cache = mediatorCacheBuilder.setExpiry(CacheConfiguration.ExpiryType.MODIFIED,
        ...
        if (inMemoryCacheSize > 0 && cache instanceof CacheImpl) {
            ((CacheImpl) cache).setCapacity(inMemoryCacheSize);
        }
        return cache;
    }
